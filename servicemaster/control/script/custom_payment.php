...
switch ($action):
	...
	$codd=$odd->DisplayOne();
		...
		$QueryObj->Where="where payment_status=1";
	...
	case'update':
			...
			if(isset($_POST['submit']) && $_POST['submit']=='Save'):
			 $QueryObj= new query('custom_order');
				...
				$QueryObj->Update();
				 Redirect(make_admin_url('custom_payment', 'list', 'update','id='.$_POST['id']));
			else:
				Redirect(make_admin_url('custom_payment','list', 'insert'));
			endif;
			break;
	case'delete':
	if(isset($_GET['orderid'])):
		...
		$QueryObj->id=$orderid;
		$QueryObj->Delete();
		Redirect(make_admin_url('custom_payment','list', 'insert','id='.$_GET['id']));
		exit;	
	endif;	
		...
		if(isset($_GET['idd'])):
			...
			$QueryObj->id=$idd;
			$QueryObj->Delete();
			Redirect(make_admin_url('custom_payment','list', 'list'));
			exit;	
		endif;
	...
	case'download':
		download_custom_orders(1);
		Redirect(make_admin_url('custom_payment'));
		break;
	default:break;
endswitch;
?>

...
switch ($action):
	case'list':
		case'list':
		$QueryObj = new query('testimonials');
	...
	case'insert':
			...
			$QueryObj->Insert();
			Redirect(make_admin_url('testimonial', 'list', 'list'));
		endif;
			break;
	case'update':
		$QueryObj = new query('testimonials');
		$QueryObj->Where="where id='".$id."'";
		$testimonials=$QueryObj->DisplayOne();
	...
	break;
	case'update2':
		$chip_info= new query('testimonials');
	   ...
	   if(isset($_POST['submit'])):
	   		...
	   		foreach($act_count as $k=>$v):
					$QueryObj1 = new query('testimonials');

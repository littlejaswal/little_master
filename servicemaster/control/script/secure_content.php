	...
	case 'update':
		...
		if(isset($_POST['save'])):
			if(is_published($id)):
				...
				$QueryObj->TableName='secure_content';
				$not=array('save');
				...
				Redirect(make_admin_url('secure_content', 'list', 'list'));
			else:
				$QueryObj->InitilizeSQL();
				$QueryObj->TableName='secure_content';
				$not=array('save');
		...
		if(isset($_POST['publish'])):
			if(is_published($id)):
				...
				if($object->preview_of_page_id):
					...
					$query= new query('secure_content');
					$query->id=$id;
					$query->Delete();
				else:
					$QueryObj->InitilizeSQL();

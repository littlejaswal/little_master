...
switch ($section):
	case 'list':
		?>
		<form action="<?=make_admin_url('image', 'update_default', 'list', 'id='.$id)?>" method="POST">
			<table width="100%" cellspacing="2" cellpadding="2" align="center" style="border:solid 1px #dcdcdc;" >
			<tr>
					<td style="text-align:left" width="30%" colspan="4"><?=make_admin_link(make_admin_url('product', 'list', 'list', 'id='.$product->parent_id), $product->name)?>&nbsp;::&nbsp;Images</td>
			...
			<?while($image=$QueryObj->GetObjectFromRecord()):?>
				<tr>
					...
					<td class="table_cell" ><img src="<?=resize_image(product_img_url_fs($image->image),100,100)?>" border="0"></td>
					<td class="table_cell" ><?=$image->position;?></td>

...
switch ($section):
	case 'list':
		...
		<table width="100%" cellspacing="0" cellpadding="2" align="center" style="border:solid 1px #dcdcdc;">
			...
			<tr>
				...
				<td class="table_cell"><a href="<?=make_admin_url('udetail','list', 'list', 'id='.$QueryObj1->id);?>"><?php echo get_control_icon('zoom')?></a></td>
				<td class="table_cell"><a href="<?php echo make_admin_url('user', 'delete', 'list', 'id='.$QueryObj1->id.'&delete=1')?>" onclick="return confirm('Are you sure? The user shall be deleted permanently');"><?php echo get_control_icon('cancel')?></a></td>
	...
	case 'insert':
		?>
		<form action="<?=make_admin_url('user', 'insert', 'list')?>" method="POST" enctype="multipart/form-data" id="register">
		<table width="100%" border="0" cellspacing="2" cellpadding="2" align="center" style="border:solid 1px #dcdcdc;">
		   <tr>
				<td colspan="2"><?=make_admin_link(make_admin_url('user', 'list', 'list'), 'Back to user listing')?></td>
		   </tr>
			<tr>
				<th colspan="2" align="center" class="table_head">Add New User</th>
			</tr>
			<tr>

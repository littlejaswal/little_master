...
switch ($section):
	case 'list':
		...
		<table width="100%" border="0" cellspacing="2" cellpadding="0" style="border:solid 1px #dcdcdc;">
			<tr>
				...
				<td align="center" width="15%" >
					<select name="id" size="1" tabindex="1">
						<?while($co=$check_options->GetObjectFromRecord()):?>
							<option value="<?=$co->id?>" <?=($id==$co->id)?'selected':'';?>><?=$co->name;?></option>
						<?endwhile;?>
					</select>
					<input type="hidden" name="Page" value="product_check">
				</td>
		...
		<table width="100%" border="0" cellspacing="2" cellpadding="0" style="border:solid 1px #dcdcdc;">
			<tr>
				<td align="right" width="50%"><?=PageControl($QueryObj->PageNo, $QueryObj->TotalPages, $QueryObj->TotalRecords, '', 'Page=product_check&id='.$id, 2);?></td>
			</tr>
		</table>
		<br/>
		<form action="<?=make_admin_url('product_check', 'update', 'list', 'id='.$id)?>" method="POST">
			<table width="100%" border="0" cellspacing="2" cellpadding="0" style="border:solid 1px #dcdcdc;">
				<tr>
				...
				<tr>
					...
					<td align="center"><input type="text" name="<?=$value['option_id']?>" value="<?=$value['position']?>" size="3"></td>
					<td align="center" colspan="2">
						<?=make_admin_link(make_admin_url('product_check', 'delete', 'list', 'option_id='.$value['option_id'].'&id='.$id.'&submit=1'),'Remove');?>
					</td>
				</tr>
				<?endforeach;;?>
				<tr>
		...
		<br/>
		<form action="<?=make_admin_url('product_check', 'insert', 'list', 'id='.$id)?>" method="POST">
			<table align="center" width="100%" border="0" cellspacing="2" cellpadding="2">
				<tr>
					<td align="left" width="100%">
						
						<b>Select Products to add to the list:</b><br/>
						<select name="product_check[]" size="10" multiple style="width:600px;">

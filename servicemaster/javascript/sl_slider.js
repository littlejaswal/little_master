var SL_Slider = new Class({
	...
	initialize: function(options) {
		...
		//remove any scrollbar(s) on the slider
		self.options.slider.setStyle('overflow', "hidden");  
	...
	slideIt: function(passedID) {
		...
		if(self.direction == 0){
			var curX = self.options.slider.getSize().x;
			var newX = (-1 * newItem.getSize().x);
            var curY = self.options.slider.getSize().y;
            var newY = (-1 * newItem.getSize().y);
		}
		else{
			var curX = (-1 * self.options.slider.getSize().x);	
			var newX = newItem.getSize().x;
            var curY = (-1 * self.options.slider.getSize().y);
